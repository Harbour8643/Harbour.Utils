...
namespace Harbour.Utils
{
    ...
    public class BarCodeHelper
    {
        ...
        /// 得到39位的条形码
        /// </summary>
        /// <param name="String">内容</param>
        /// <param name="width">宽度</param>
        /// <param name="height">高度</param>
        /// <returns></returns>
        public static string Get39(string String, int width, int height)
        {
        ...
        /// <param name="height">高度</param>
        /// <returns></returns>
        public static string GetEAN13(string String, int width, int height)
        {
            int checkcode_input = -1;//输入的校验码
            if (!Regex.IsMatch(String, @"^\d{12}$"))
            {
                if (!Regex.IsMatch(String, @"^\d{13}$"))
                {
                    return "存在不允许的字符！";
                }
                else
                {
                    checkcode_input = int.Parse(String[12].ToString());
            ...
            for (int i = 0; i < 12; i++)
            {
                if (i % 2 == 0)
                {
                    sum_odd += int.Parse(String[i].ToString());
                }
                else
                {
                    sum_even += int.Parse(String[i].ToString());
                }
            }
            ...
            String += checkcode;//变成13位

            // 000000000101左侧42个01010右侧35个校验7个101000000000
            // 6        101左侧6位 01010右侧5位校验1位101000000000
            StringBuilder sb = new StringBuilder();//二进制串
            sb.Append("000000000101");

            string type = Ean13Type(String[0]);
            for (int i = 1; i < 7; i++)
            {
                sb.Append(Ean13(String[i], type[i - 1]));
            }
            sb.Append("01010");
            for (int i = 7; i < 13; i++)
                sb.Append(Ean13(String[i], 'C'));

            sb.Append("101000000000");
            string result_html = ""; //HTML代码
            string color = "";       //颜色
            ...
            result_html += "<div style=\"float:left;color:#000000;width:" + (width * 9) + "px;text-align:center;\">" + String[0] + "</div>";
            ...
            for (int i = 1; i < 7; i++)
            {
                result_html += "<div style=\"float:left;width:" + (width * 7) + "px;color:#000000;text-align:center;\">" + String[i] + "</div>";
            }
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#FFFFFF;\"></div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#000000;\"></div>";
            result_html += "<div style=\"float:left;width:" + width + "px;height:" + height_bottom + "px;background:#FFFFFF;\"></div>";
